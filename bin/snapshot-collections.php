...
$log = new SyncLogger('snapshot', 'cron');
...
try {
    ...
    foreach ($users as $userId) {
        $stmt = $db->prepare(
            "SELECT
                ...
                COALESCE(SUM(COALESCE(c.price_en, c.cardmarket_price, 0) * uc.quantity), 0) as total_value_eur
             FROM user_cards uc
    ...
    $log->success("Snapshots for $count users", ['users' => $count]);
    echo "Snapshots created for $count users\n";
} catch (\Throwable $e) {
