<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-display font-bold text-white"><?= t('market.title') ?></h1>
        <p class="text-sm text-dark-400 mt-1"><?= t('market.subtitle') ?></p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" x-data="priceMovers()" x-init="load()">
        <div class="glass rounded-2xl p-6">
            <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
                <i data-lucide="trending-up" class="w-5 h-5 text-green-400"></i> <?= t('market.top_gainers') ?>
            </h2>
            <div class="space-y-2">
        ...
        <div class="glass rounded-2xl p-6">
            <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
                <i data-lucide="trending-down" class="w-5 h-5 text-red-400"></i> <?= t('market.top_losers') ?>
            </h2>
            <div class="space-y-2">
    ...
    <div class="glass rounded-2xl p-6">
        <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
            <i data-lucide="gem" class="w-5 h-5 text-gold-400"></i> <?= t('market.most_expensive') ?>
        </h2>
        <?php if (empty($expensive)): ?>
            <p class="text-sm text-dark-400 text-center py-4"><?= t('market.no_price') ?></p>
        <?php else: ?>
    ...
    <div class="glass rounded-2xl p-6">
        <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
            <i data-lucide="heart" class="w-5 h-5 text-red-400"></i> <?= t('market.most_collected') ?>
        </h2>
        <?php if (empty($collected)): ?>
            <p class="text-sm text-dark-400 text-center py-4"><?= t('market.no_collection_data') ?></p>
        <?php else: ?>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead><tr class="text-xs text-dark-400 uppercase border-b border-dark-600">
                        <th class="text-left pb-3 pr-4">#</th><th class="text-left pb-3 pr-4"><?= t('market.card') ?></th><th class="text-right pb-3 pr-4"><?= t('market.collectors') ?></th><th class="text-right pb-3 pr-4"><?= t('market.total_owned') ?></th><th class="text-right pb-3"><?= t('market.price') ?></th>
                    </tr></thead>
                    <tbody>
    ...
    <div class="glass rounded-2xl p-6">
        <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
            <i data-lucide="package" class="w-5 h-5 text-purple-400"></i> <?= t('market.set_summary') ?>
        </h2>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <?php $curSym = \App\Core\Currency::symbol(); $curLbl = \App\Core\Currency::label(); ?>
                <thead><tr class="text-xs text-dark-400 uppercase border-b border-dark-600">
                    <th class="text-left pb-3 pr-4"><?= t('cards.set') ?></th><th class="text-right pb-3 pr-4"><?= t('home.cards') ?></th><th class="text-right pb-3 pr-4"><?= t('market.total') ?><?= $curLbl ?>)</th><th class="text-right pb-3"><?= t('market.avg_price') ?></th>
                </tr></thead>
                <tbody>
    ...
    <div class="glass rounded-2xl p-6">
        <h2 class="text-lg font-display font-bold text-white flex items-center gap-2 mb-4">
            <i data-lucide="sparkles" class="w-5 h-5 text-cyan-400"></i> <?= t('market.recently_added') ?>
        </h2>
